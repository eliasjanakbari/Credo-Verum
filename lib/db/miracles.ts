...
import type { EvidenceSource, MiracleCategoryType, GospelReferences, EvidencePassage } from '../types/sources';
  ...
  for (const row of rows) {
    ...
    if (row.ManuscriptShelfmark && !evidence.manuscripts.some(m => m.shelfmark === row.ManuscriptShelfmark)) {
      evidence.manuscripts.push({
        ...
        highlightImageUrl: row.ManuscriptHighlightImageURL || undefined,
...
export async function getAllMiracles(): Promise<EvidenceSource[]> {
  ...
  const result = await pool.request().query(`
    SELECT
      ...
      mw.HighlightImageURL as ManuscriptHighlightImageURL,
